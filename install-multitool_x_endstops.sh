...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/tool_x_endstop.py" "${KLIPPER_PATH}/klippy/extras/tool_x_endstop.py"
    ln -sf "${SRCDIR}/tool_x_router.py" "${KLIPPER_PATH}/klippy/extras/tool_x_router.py"
    echo "[OK]"
}
...
# Add updater for multitool_leds to moonraker.conf
add_updater()
{
    echo -e -n "Adding update manager to moonraker.conf... "

    update_section=$(grep -c '\[update_manager multitool_x_endstops\]' ${MOONRAKER_CONFIG_DIR}/moonraker.conf || true)
    if [ "${update_section}" -eq 0 ]; then
        ...
        echo -e "[update_manager multitool_x_endstops] already exists in moonraker.conf [SKIPPED]"
    fi
}
...
uninstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/tool_x_endstop.py" ]; then
        echo -n "Uninstalling... "
        rm -f "${KLIPPER_PATH}/klippy/extras/tool_x_endstop.py"
        rm -f "${KLIPPER_PATH}/klippy/extras/tool_x_router.py"
        echo "[OK]"
        echo "You can now remove the [update_manager multitool_x_endstops] section in your moonraker.conf and delete this directory. Also remove all relevant configurations from your Klipper configuration."
    else
        echo "tool_x_endstop.py not found in \"${KLIPPER_PATH}/klippy/extras/\". Is it installed?"
        echo "[FAILED]"
    fi
}
